...
std::vector<std::pair<size_t, size_t>> hungarianTrack(std::vector<float> &box, \
  ...
  //   {82.0f, 83.0f, 69.0f}, {77.0f, 37.0f, 49.0f}, {11.0f, 69.0f, 5.0f}, {8.0f, 9.0f, 98.0f}
  // };
  std::vector<std::vector<float>> association_mat;
  std::vector<float> iouVector;
  for (auto it : afterNMSBoxVector) {iouVector.push_back(func(box, it));}
  association_mat.push_back(iouVector);
