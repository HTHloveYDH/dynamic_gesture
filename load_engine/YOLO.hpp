...
bool YOLO::processOpenCVInput_(const cv::Mat &img, const samplesCommon::BufferManager &buffers) {
  ...
  cv::Mat resized_img;
  cv::resize(img, resized_img, cv::Size(inputSize_, inputSize_), (0.0), (0.0), 0);  // interpolation: INTER_LINEAR
    ...
    for (int c = 0, volChl = inputH_ * inputW_; c < inputC_; c++) {
      for (int h = 0; h < inputH_; h++) {
        for (int w = 0; w < inputW_; w++) {
          hostDataBuffer[b * volImg + c * volChl + h * inputW_ + w] = croped_mat.at<uchar>(h, w) / 255.0;
        }
      }
    }
  }
