...
bool resize_image(const void *src, int src_wdith, int src_height,
                  int width_step, void *dst_buf, int dst_width,
                  int dst_height) {
  if (!src || !dst_buf) {
    return false;
  }
  ...
  NPP_CHECK(nppiResize_8u_C3R((Npp8u *)src, width_step, srcsize, srcroi,
                              (Npp8u *)dst_buf, dst_width * 3, dstsize, dstroi,
                              NPPI_INTER_LINEAR));
  return true;
}
...
bool padding_image(const void *src, int src_wdith, int src_height,
  ...
  NPP_CHECK(nppiCopyConstBorder_8u_C3R(
      (Npp8u *)src, width_step, oSrcSizeROI, (Npp8u *)dst_buf, dst_wid * 3,
...
bool normlize_image(float *src, int src_wdith, int src_height, int channel,
  ...
  NPP_CHECK(nppiSubC_32f_C3IR(mean, (Npp32f *)src,
                              src_wdith * channel * sizeof(float), dstsize));

  NPP_CHECK(nppiDivC_32f_C3IR(std_val, (Npp32f *)src,
                              src_wdith * channel * sizeof(float), dstsize));
  return true;
}
...
bool crop_image(const unsigned char *src, int src_wdith, int src_height,
  ...
  NPP_CHECK(nppiCopy_8u_C3R((Npp8u *)(src + offset), width_step,
                            (Npp8u *)dst_buffer, roi.width * src_channel,
                            dst_size));
  return true;
}